...
import { OrganizationProjectDetail, OrganizationProjectMilestone, OrganizationProjectDependency, OrganizationProjectProgress } from '../../../core/models/organization.model';
...
export class OrganizationProjectDetailComponent implements OnInit {
  ...
  load(): void {
    ...
    this.organization.getProgramDetail(id).subscribe({
      next: response => { this.project = response.data || null; this.loading = false; this.loadProgress(); this.loadDependencies(); },
      error: error => {
