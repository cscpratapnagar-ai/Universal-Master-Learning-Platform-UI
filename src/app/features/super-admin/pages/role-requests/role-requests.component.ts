...
export class RoleRequestsComponent implements OnInit {
  ...
  approve(request:RoleRequest):void{if(this.processing)return;this.processing=request.id;this.service.approve(request.id).subscribe({next:()=>{this.notice=request.userName+' is now approved for '+request.requestedRole+'.';this.processing='';this.load();},error:e=>{this.processing='';this.error=e?.error?.message||'Unable to approve this request.';}});}
  reject(request:RoleRequest):void{if(this.processing)return;const reason=window.prompt('Optional rejection reason:','');if(reason===null)return;this.processing=request.id;this.service.reject(request.id,reason).subscribe({next:()=>{this.notice=request.userName+' request was rejected.';this.processing='';this.load();},error:e=>{this.processing='';this.error=e?.error?.message||'Unable to reject this request.';}});}
