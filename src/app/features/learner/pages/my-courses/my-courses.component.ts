...
ngOnInit(){const u=this.auth.currentUser(); if(!u){this.router.navigateByUrl('/auth/login');return;} this.learning.myCourses().subscribe({next:r=>{this.courses=r.data||[];this.loading=false;},error:()=>{this.error='Unable to load your courses.';this.loading=false;}});}
