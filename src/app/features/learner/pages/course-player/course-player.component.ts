...
export class CoursePlayerComponent implements OnInit {
  ...
  error=''; completing=false; certificate?: StudentCertificate; certificateLoading=false; issuingCertificate=false; certificateMessage='';
  ...
  constructor(private route:ActivatedRoute,private learning:LearningService,private assessments:AssessmentService,private certificates:CertificateService,private router:Router){}
  ...
  load(){this.loading=true;this.error='';const currentLessonId=this.selected?.id;this.learning.courseLearning(this.enrollmentId).subscribe({next:r=>{this.data=r.data;const all=this.allLessons;const requestedLessonId=this.route.snapshot.queryParamMap.get('lessonId');this.selected=(currentLessonId?all.find(l=>l.id===currentLessonId&&!l.locked):undefined)||(requestedLessonId?all.find(l=>l.id===requestedLessonId&&!l.locked):undefined)||all.find(l=>!l.completed&&!l.locked)||all[0];this.loading=false;this.loadAssessment();this.autoCompleteIfNeeded();this.syncCertificateState();},error:e=>{this.loading=false;this.error=e?.error?.message||'We could not load this course right now.';}});}
  select(l:Lesson){if(l.locked)return;this.selected=l;this.error='';this.loadAssessment();this.autoCompleteIfNeeded();window.scrollTo({top:0,behavior:'smooth'});}
  ...
  autoCompleteIfNeeded():void{if(!this.selected||this.selected.completed||this.selected.locked||this.completing)return;if(this.selected.completionMode!=='AUTO_COMPLETE')return;if(this.prerequisiteLessons.length)return;this.complete(true);}
  complete(fromAuto=false){if(!this.selected||this.selected.completed||this.selected.locked||this.completing)return;if(this.prerequisiteLessons.length)return;if(!fromAuto&&this.selected.completionMode==='ASSESSMENT_REQUIRED'&&(!this.assessment||!this.assessment.passed)){this.openAssessment();return;}this.completing=true;this.error='';this.learning.completeLesson(this.enrollmentId,this.selected.id).subscribe({next:()=>{this.completing=false;this.load();},error:e=>{this.completing=false;this.error=e?.error?.message||'Unable to complete this lesson. Please try again.';}});}
  syncCertificateState(): void {
    if (!this.data || Number(this.data.progressPercent || 0) < 100) { this.certificate = undefined; return; }
    this.certificateLoading = true;
    this.certificates.mine().subscribe({
      next: r => { this.certificate = (r.data || []).find(c => c.courseId === this.data?.courseId); this.certificateLoading = false; },
      error: () => { this.certificateLoading = false; }
    });
  }
  issueCertificate(): void {
    if (!this.data || this.issuingCertificate || this.certificate) return;
    this.issuingCertificate = true; this.certificateMessage = '';
    this.certificates.issue(this.data.courseId).subscribe({
      next: r => { this.certificate = r.data; this.issuingCertificate = false; this.certificateMessage = 'Certificate issued successfully.'; },
      error: e => { this.issuingCertificate = false; this.certificateMessage = e?.error?.message || 'Certificate could not be issued right now.'; }
    });
  }
