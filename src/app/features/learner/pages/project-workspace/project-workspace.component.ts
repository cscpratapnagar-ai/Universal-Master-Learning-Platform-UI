...
export class ProjectWorkspaceComponent implements OnInit {
  project:any=null; milestones:any[]=[]; loading=true; error='';
  ...
  ngOnInit():void{const id=this.route.snapshot.paramMap.get('programId');if(!id){this.error='Project not found.';this.loading=false;return;}this.learning.myProjectWorkspace(id).subscribe({next:r=>{this.project=r.data;this.learning.myProjectExecution(id).subscribe({next:x=>this.milestones=x.data?.milestones||[]});this.loading=false;},error:e=>{this.error=e?.error?.message||'Unable to load project workspace.';this.loading=false;}})}
