...
export class LearningService {
  ...
  myProjectExecution(programId:string):Observable<ApiResponse<OrganizationProjectEnrollment & {milestones:{id:string;title:string;description?:string;dueDate?:string|null;sortOrder:number;status:string;overdue:boolean}[]}>>{return this.http.get<ApiResponse<OrganizationProjectEnrollment & {milestones:{id:string;title:string;description?:string;dueDate?:string|null;sortOrder:number;status:string;overdue:boolean}[]}>>(`${this.base}/program-enrollments/mine/${encodeURIComponent(programId)}/execution`);}
  ...
  updateAssessment(assessmentId:string,request:{title:string;passingScore:number;maxAttempts?:number}):Observable<ApiResponse<{id:string;title:string;level:string;passingScore:number;maxAttempts:number}>>{return this.http.put<ApiResponse<{id:string;title:string;level:string;passingScore:number;maxAttempts:number}>>(`${this.base}/assessments/${encodeURIComponent(assessmentId)}`,request);}
  updateAssessmentQuestion(assessmentId:string,questionId:string,request:{questionText:string;questionType:string;points:number;difficultyLevel?:string;options:{text:string;correct:boolean}[]}):Observable<ApiResponse<{id:string;assessmentId:string}>>{return this.http.put<ApiResponse<{id:string;assessmentId:string}>>(`${this.base}/assessments/${encodeURIComponent(assessmentId)}/questions/${encodeURIComponent(questionId)}`,request);}
  ...
  createLessonAssessment(lessonId:string,request:{title:string;passingScore:number}):Observable<ApiResponse<{id:string;title:string}>>{return this.http.post<ApiResponse<{id:string;title:string}>>(`${this.base}/assessments/lessons/${encodeURIComponent(lessonId)}`,request);}
